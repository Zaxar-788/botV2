...
def test_fixed_pairs_fetcher():
    ...
    # Тест 6: Асинхронная загрузка
    print("\n🔄 Тест 6: Асинхронное обновление")
    
    async def test_async_update():
        start_time = time.time()
        success = await fetcher.update_pairs_async()
        end_time = time.time()
        
        if success:
            print(f"✅ Асинхронное обновление завершено за {end_time - start_time:.2f} сек")
            pairs_after_update = fetcher.get_all_pairs()
            print(f"   Количество пар после обновления: {len(pairs_after_update)}")
        else:
            print("❌ Асинхронное обновление не удалось")
